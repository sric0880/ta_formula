...
def np_ext():
    cdef int[:] a = np.array([1, 2, 4, 5], dtype=np.intc)
    ...
    cdef double[:] a1 = np.array([NaN, 2.0, 4.0, 5.0], dtype=np.double)
    b = ta.rolling_sum(a1, 2)
    np.testing.assert_array_equal(b, [NaN,NaN,6,9])
    cdef double[:] a12 = np.array([NaN, NaN, 4.0, 5.0], dtype=np.double)
    b = ta.rolling_sum(a12, 3)
    np.testing.assert_array_equal(b, [NaN,NaN,NaN,NaN])
    cdef double[:] a11 = np.array([1, 2.0, 4.0, NaN], dtype=np.double)
    b = ta.rolling_sum(a11, 2)
    np.testing.assert_array_equal(b, [NaN,3,6,NaN])
    cdef double[:] a111 = np.array([1, 2.0, NaN, 3, 4.0, 5.0], dtype=np.double)
    b = ta.rolling_sum(a111, 3)
    np.testing.assert_array_equal(b, [NaN,NaN,NaN,NaN,NaN,12])
    cdef long long[:] a2 = np.array([1, -2, 3, -4], dtype=np.longlong)
